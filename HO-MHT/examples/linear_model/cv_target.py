...
class TargetPosition_CV2D(Target):
    ...
    _motion = ConstantVelocity2D(sigma=5)
    _measure = ConstantVelocity(sigma=0.5)
    print(_motion.Q())
    print(_measure.R())
    ...
    def _P0(cls):
        # R = cls._measure.R()
        # print(np.diag([R[0,0], R[1,1], 1.0, 1.0]))
        # return np.diag([R[0,0], R[1,1], 1.0, 1.0])
        # print(np.diag([100, 100, 100, 100]))
        return np.diag([100, 100, 100, 100])
